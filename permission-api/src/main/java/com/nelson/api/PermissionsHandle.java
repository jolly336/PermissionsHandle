...
public final class PermissionsHandle {
    ...
    private static void _requestPermissions(Object object, int requestCode, String[] permissions) {
        if (!Utils.isOverMarshmallow()) {
            doExecuteSucess(object, requestCode);
        }
        ...
        if (deniedPermissions.size() > 0) {
            if (object instanceof Activity) {
                ...
                throw new IllegalArgumentException(String.format("%s is not supported!", object.getClass().getName()));
            }
        } else {
            doExecuteSucess(object, requestCode);
        }
    ...
    private static void doExecuteSucess(Object activity, int requestCode) {
    ...
    private static PermissionProxy findPermissionProxy(Object activity) {
        ...
        throw new RuntimeException(String.format("cannot find %s ,something when compiler.", activity.getClass().getSimpleName() + SUFFIX));
    }
}
